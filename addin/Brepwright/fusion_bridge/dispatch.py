...
# ── Simple logging (replaces frankhommers' fusionAddInUtils) ───────────

def _futil_log(message: str, level=None):
    """Simple logging to Fusion's text commands palette."""
    try:
        print(f"[Brepwright] {message}")
    except Exception:
        pass
    try:
        with open(settings.LOG_FILE, "a", encoding="utf-8") as log_file:
            stamp = datetime.utcnow().isoformat(timespec="seconds")
            log_file.write(f"{stamp}Z {message}\n")
    except Exception:
        pass


def _futil_log_level(message: str, level):
    """Log with level."""
    _futil_log(message)
...
def log(message: str, level=None):
    ...
    if threading.current_thread() is threading.main_thread():
        _futil_log(message) if level is None else _futil_log_level(message, level)
        return
    with _msg_lock:
...
def drain_logs():
    ...
    for text, lvl in snapshot:
        _futil_log(text) if lvl is None else _futil_log_level(text, lvl)
