...
def execute_python(args):
    ...
    try:
        ...
        for k, v in exec_globals.items():
            if not k.startswith("_") and k not in ("adsk",):
                session_ns[k] = v
...
def fetch_design_guide(args):
    """Read the bundled Fusion design guide."""
    # Try to read from the guides directory
    addin_dir = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
    guide_path = os.path.join(addin_dir, "guides", "design_guide.md")

    if os.path.exists(guide_path):
        with open(guide_path, "r") as f:
            return f.read()

    # Fallback: return a summary of best practices
    return """# Fusion 360 Design Guide (Summary)

## Key API Patterns
- Use `createInput()` + setter methods + `add()` pattern for features
- Fusion API uses cm internally (divide mm by 10.0)
- Name bodies immediately after creation
- Use `ObjectCollection` for multi-entity operations
- Thread safety: API calls must be on main thread

## Common Pitfalls
- `addNewComponent(None)` fails → use `Matrix3D.create()`
- `JointGeometry.create()` doesn't exist → use `createByPoint()`
- `contactSets` is on `design`, not `rootComp`
- Torus: use SweepFeatures (not RevolveFeatures)
- STL export: pass BRepBody, not Component

See LIBRARY.md for the full API pitfall reference.
"""
