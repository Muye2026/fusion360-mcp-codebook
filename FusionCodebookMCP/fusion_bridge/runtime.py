...
def create_server():
    ...
    SERVER_INFO["name"] = "fusion-codebook-mcp"
...
def _start_server():
    ...
    log(f"[OK] Fusion Codebook MCP server running at http://127.0.0.1:{settings.MCP_SERVER_PORT}/mcp")
...
def start():
    ...
    log("Fusion Codebook MCP starting...")
    ...
    if settings.MCP_AUTO_CONNECT:
        try:
            if not _start_server():
                stop_main_thread_dispatch()
                return False
            log("Fusion Codebook MCP started successfully")
        except Exception as exc:
...
def stop():
    ...
    log("Fusion Codebook MCP stopping...")
    stop_main_thread_dispatch()
    ...
    log("Fusion Codebook MCP stopped")
    drain_logs()
